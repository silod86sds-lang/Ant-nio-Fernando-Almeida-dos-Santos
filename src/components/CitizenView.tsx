...
import { ArrowLeft, User, AlertCircle, FileText, ShieldAlert, CheckCircle2 } from 'lucide-react';
import { cn } from '../lib/utils';
import { infractionsList } from '../store/mockData';

export default function CitizenView({ bi, onBack }: { bi: string, onBack: () => void }) {
  const { citizensData, infractionsData, addInfraction, updateCitizenLicense, currentUser } = useAppContext();
  const citizen = citizensData.find(c => c.bi === bi);
  ...
  if (!citizen) {
    return (
      <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center justify-center">
        ...
        <h2 className="text-xl font-bold text-slate-800 mb-2">Cidadão não encontrado</h2>
        <p className="text-slate-500 mb-6">O BI {bi} não consta na base de dados.</p>
  ...
  const citizenInfractions = infractionsData.filter(i => i.citizenBi === bi);
  const hasOverdue = citizenInfractions.some(i => i.status === 'Em atraso');

  const handleSuspendLicense = () => {
    if (window.confirm('Tem certeza que deseja suspender a carta deste condutor?')) {
      updateCitizenLicense(bi, 'Suspensa');
    }
  };

  const handleSubmitInfraction = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedInfraction || !currentUser) return;

    const infDetails = infractionsList.find(i => i.id === selectedInfraction);
    if (!infDetails) return;

    // Check for recent warnings of same type
    if (infractionType === 'warning') {
      const recentWarning = citizenInfractions.find(i => 
        i.type === 'warning' && 
        i.description === infDetails.description &&
        (new Date().getTime() - new Date(i.date).getTime()) / (1000 * 3600 * 24) <= 21
      );

      if (recentWarning) {
        alert('ATENÇÃO: Este cidadão já recebeu um aviso para esta infração nos últimos 21 dias. O sistema irá gerar uma multa automaticamente.');
        setInfractionType('fine');
        return; // Force them to submit again as fine
      }
    }
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {hasOverdue && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <h3 className="text-red-800 font-semibold">Atenção: Multas em Atraso</h3>
              <p className="text-red-700 text-sm mt-1">Este cidadão possui multas não pagas fora do prazo.</p>
              {citizen.licenseStatus === 'Válida' && (
                <button 
                ...
                >
                  Suspender Carta Automaticamente
                </button>
              )}
            </div>
          </div>
        ...
        {citizen.licenseStatus === 'Suspensa' && (
          <div className="bg-red-600 text-white rounded-xl p-4 flex items-center gap-3 shadow-sm">
            <AlertCircle size={24} />
            <div>
              <h3 className="font-bold text-lg">CONDUÇÃO ILEGAL</h3>
              <p className="text-red-100 text-sm">A carta deste condutor encontra-se suspensa. Sujeito a processo crime e apreensão do veículo.</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start">
            <img src={citizen.photo} alt={citizen.name} className="w-24 h-24 rounded-xl object-cover border border-slate-200 shadow-sm" />
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-slate-900">{citizen.name}</h1>
              <p className="text-slate-500 font-mono mt-1">BI: {citizen.bi}</p>
              
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-1">Estado da Carta</p>
                  <p className={cn(
                    "font-semibold",
                    citizen.licenseStatus === 'Válida' ? "text-emerald-600" : 
                    citizen.licenseStatus === 'Suspensa' ? "text-red-600" : "text-amber-600"
                  )}>
                ...
                <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                  ...
                  <p className="font-semibold text-slate-900">{citizen.licenseCategory}</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-1">Validade</p>
                  <p className="font-semibold text-slate-900">{new Date(citizen.licenseExpiry).toLocaleDateString('pt-PT')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-200">
            <h2 className="text-lg font-bold text-slate-900">Histórico de Infrações</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {citizenInfractions.length === 0 ? (
              <div className="p-8 text-center text-slate-500">Nenhum registo encontrado.</div>
            ) : (
              citizenInfractions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(inf => (
